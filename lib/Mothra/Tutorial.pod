...
if you have one, from C<.gitignore>.
...
and if it was successful, you get a build, and also new C<Build.PL>,
C<META.json> and C<README.md> in your working directory for
git-friendliness. C<git add> them and commit it.

    > git add Build.PL META.json README.md && git commit -m "git stuff"

Now you're ready to roll a new release with Mothra.

    > moth bumpversion
    > moth release

to make a new release, in the same way described above for a new Mothra setup.
